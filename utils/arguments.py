    ...
    if isinstance(v, list) or isinstance(v, tuple):
        ...
        if v.lower() in ['yes', 'true', 't', 'y', 'no', 'false', 'f', 'n']:
            ...
            if c is None:
                ...
                v = [process_arg(v_) for v_ in v[1:-1].split(c)]
        try:
            vf = float(v)
            if '.' in v:
                return vf
